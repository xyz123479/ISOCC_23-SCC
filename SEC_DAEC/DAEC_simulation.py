...
# Error correction capability of this scheme is SSC : can correct 1-bit error and 1-cell error
...
def correction_DAEC(codeword, syndrome, H_matrix, Sum_matrix) :
...
def main() :
    H_matrix = np.loadtxt("DAEC_H_matrix.txt", dtype = "int")
    Sum_matrix_T = np.loadtxt("DAEC_Sum_matrix_T.txt", dtype = "int")
    ...
    print("-------------------------------------")
    print(" SEC-DED-DAEC Code configuration ({0}, {1})".format(codeword_len, data_len))
    print("-------------------------------------")
    ...
    for Nrun in range (0, 1000000) :
        ...
        corrected_codeword, nonzero, correctable = correction_DAEC(codeword, syndrome, H_matrix, Sum_matrix)
